...
exports.addItemToCart = async (req, res, next) => {
  try {
    ...
    return next(
      new ApiError(500, `Lỗi khi thêm sản phẩm vào giỏ hàng ${error}`)
    );
  }
};
...
exports.getCart = async (req, res, next) => {
  try {
    ...
    return next(
      new ApiError(500, `Lỗi khi lấy sản phẩm vào giỏ hàng ${error}`)
    );
  }
};
...
exports.updateItemInCart = async (req, res, next) => {
  try {
    ...
    return next(new ApiError(500, `Lỗi khi cập nhật số lượng: ${error}`));
  }
};
